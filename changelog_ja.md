## 0.0.365 - 2025-11-29
- Copilot CLIのバージョン情報表示を改善しました。
- changelogの日本語訳を最新化しました。
- その他細かな修正。

## 0.0.363 - 2025-11-24

- GitHub Copilot CLIでOpus 4.5、GPT-4.1、GPT-5-Miniが利用可能になりました
- 画像データの貼り付け時に、ファイルアイコンではなく画像ファイルの内容を優先的に貼り付けるようになりました
- シェルツール名のタイムラインレンダリングを改善しました
- 認証用のGITHUB_ASKPASS環境変数のサポートを追加しました
- MCPサーバーが`--prompt`モードで動作するようになりました

## 0.0.362 - 2025-11-20

- Windows上での画像のドラッグ&ドロップに関する問題を修正しました
- シェルコマンドがBashおよびPowerShellの履歴ファイルに含まれないようになりました
- クリップボードから画像データをCLIに直接貼り付けられるようになりました
- プロンプトとツール指示のクリーンアップと更新により、より流暢になりました

## 0.0.361 - 2025-11-18

- GitHub Copilot CLIでGemini 3 Proが利用可能になりました

## 0.0.360 - 2025-11-18

- ユーザー権限を待機している間にファイル操作がタイムアウトする問題を修正しました

## 0.0.359 - 2025-11-17

- ドラッグ&ドロップおよび画像ファイルへのパスの貼り付けによるコンテキストへの画像追加をサポートしました。入力ボックスでの画像スラッグのレンダリング方法を改善しました
- セッションをMarkdownファイルまたはGitHub Gistとして保存する`/share`コマンドを追加しました
- セッション終了時にキャッシュされたトークンがゼロと表示されるバグを修正しました
- PATHから`ripgrep`を任意で使用するための`USE_BUILTIN_RIPGREP`環境変数を有効にしました
- リモートリポジトリのデフォルトブランチからカスタムエージェントを取得する際に、エージェントのローカルコピーが使用されているかどうかについての混乱を招く問題を修正しました
- カスタムエージェントの設定の問題を修正しました
- `Ctrl+C`のパフォーマンスを改善しました
- ツール引数の解析の安全性を改善しました
- ツール名とパスを区別し、ツールの成功/エラーアイコンを改善しました
- `copilot -p`は権限リクエストについてインタラクティブなプロンプトを表示しなくなりました
- ツールの説明から不要な空白を削除しました
...
- 認証用の`COPILOT_GITHUB_TOKEN`環境変数のサポートを追加しました（`GH_TOKEN`より優先されます）
- コマンド外のヒアドキュメント処理を改善し、シェルコマンドの安全性を向上させました
- diffハンク行がdiffボックスの幅を適切に埋めるようになりました
- GitHub Actions環境のMCPサーバーは、作業ディレクトリとして自動的に`GITHUB_WORKSPACE`を使用するようになりました
- ローカル変更が存在しない場合でも`/delegate`コマンドが正しく動作するようになりました
- ファイル名に特殊文字を含むカスタムエージェントが失敗しなくなりました
- `/model`コマンドでサポートされていないモデルを使用する際のエラーメッセージを改善しました
- 異なるOpenAIベースURLを使用する場合、代替モデルプロバイダーが正しく動作するようになりました

## 0.0.353 - 2025-10-28

- カスタムエージェントのサポートを追加しました。カスタムエージェント定義は、`~/.copilot/agents`、リポジトリ内の`.github/agents`、または組織の`.github`リポジトリから取得されます。インタラクティブに`/agent`スラッシュコマンドで、または非インタラクティブに`--agent <agent>`で明示的にエージェントを呼び出すことができます。エージェントは、タスクの完了中にモデルが呼び出すことができるツールとしても提供されます
- `/delegate`コマンドを追加して、タスクをCopilotコーディングエージェントに非同期で委任できるようにしました。ステージされていない変更は新しいブランチにコミットされ、GitHubリポジトリでPRが開かれ、Copilotがバックグラウンドで作業を完了します。

## 0.0.352 - 2025-10-27

- スラッシュを含むMCPツールの処理を改善しました
- サポートされていないモデルを使用する際の`/model <model>`コマンドからのエラーメッセージを改善しました
